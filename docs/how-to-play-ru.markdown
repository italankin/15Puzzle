...
По умолчанию в качестве отсутствующей клетки будет последний номер (например, для 4x4 это 16, 9 для 3x3, 12 для 4x3 и так далее). Если выбрано `случайно`, отсутствующий номер каждый раз будет выбираться случайно.
